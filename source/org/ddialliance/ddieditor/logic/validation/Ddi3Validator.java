...
public class Ddi3Validator {
	static Log log = LogFactory.getLog(LogType.SYSTEM, Ddi3Validator.class.getName());
	...
	private static boolean checkIdUniqeness(String id, String elementType, String parentId,
	String parentType) throws Exception {		
		StringBuffer query = new StringBuffer();
		query.append("for $validation in ");
		query.append(PersistenceManager.getInstance().getResourcePath());
		query.append("/");
		query.append(DdiManager.getInstance().addFullyQualifiedNamespaceDeclarationToElements(elementType));
		query
		...
		boolean idTest = PersistenceManager.getInstance().querySingleBoolean(query
				.toString());
