...
public class UrnUtilTest extends DdieditorTestCase {
	...
	public void getUrn() throws Exception {
		...
		Urn urn = Urn2Util.getUrn("datacollection__DataCollection", "dd", null,
				null, null, null);

		urn = Urn2Util.getUrn("QuestionItem", "qi_1", "", "qs_", "", "dda.dk");
		Assert.assertEquals(
				"urn:ddi:dda.dk.ddi:QuestionScheme.qs_.1.0.0:QuestionItem.qi_1.1.0.0",
				urn.toUrnString());

		urn = Urn2Util.getUrn("DDIInstance", "dda-4755", "1.0", null, null,
				null);
		Assert.assertEquals("urn:ddi:3.0:DDIInstance=dda.dk:dda-4755(1.0)",
				urn.toUrnString());

		urn = Urn2Util.getUrn("Category", "cat_1", "", "cats_40", "", null);
		Assert.assertEquals(
				"urn:ddi:3.0:Category=dda.dk:dda-4755(1.0).cat_1(1.0)",
				urn.toUrnString());

		PersistenceManager.getInstance().setWorkingResource(
				DdieditorTestCase.SINGLE_MANINTAINABLE_QS_FD_NS_DOC);
		urn = Urn2Util.getUrn("QuestionScheme", "qs_", "1.0", null, null, null);
		Assert.assertEquals("urn:ddi:3.0:QuestionScheme=dda.dk:qs_(1.0)",
				urn.toUrnString());
	}
	...
	public void getObjectByUrn() throws Exception {
		...
		LightXmlObjectType lightXmlObject = Urn2Util
				.getLightXmlObjectByUrn(urn);
		Assert.assertEquals("qi_1", lightXmlObject.getId());

		lightXmlObject = DdiManager.getInstance()
				.getQuestionSchemesLight(null, null, null, null)
				.getLightXmlObjectList().getLightXmlObjectList().get(2);

		urn = Urn2Util.getUrn(lightXmlObject);
		Assert.assertEquals(
				"urn:ddi:3.0:QuestionScheme=hungobongo:qs_-2(1.0.2)",
				urn.toUrnString());
	}
}
